...
function saveData(data) {
  ...
  for (const cat of ["results", "admitCards", "latestJobs", "answerKeys", "documents", "admissions"]) {
    if (cleanData[cat]) {
      cleanData[cat] = cleanData[cat].map((it) => {
        const { _processed, ...rest } = it;
        return rest;
      });
    }
  }
...
async function main() {
  ...
  if (newCount > 0) {
    ...
    for (const cat of ["results", "admitCards", "latestJobs", "answerKeys", "documents", "admissions"]) {
      for (const item of (existing[cat] || [])) {
        ...
        if (post) {
          ...
          post.slug = slug;
          const filePath = resolve(POSTS_DIR, `${slug}.json`);
          let existingPost = null;
          if (existsSync(filePath)) {
            try {
              existingPost = JSON.parse(readFileSync(filePath, "utf8"));
            } catch {}
          }
          ...
          existing.posts[slug] = {
            ...(existing.posts[slug] || {}),
            lastDate: post.importantDates?.find((d) => d.toLowerCase().includes("last"))?.replace(/^.*?:/, "").trim(),
            publishedDate,
            publishedAt,
          };
    ...
    console.log("Updated data/scraped-data.ts and data/scraped.json");
  } else {
