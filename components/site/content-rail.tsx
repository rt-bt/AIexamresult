...
export function ContentRail({
  ...
  const gridClass = variant === "carousel"
        ...
        <motion.div variants={container} initial="hidden" whileInView="show" viewport={{ once: true, margin: "-30px" }} className={gridClass}>
          {items.map((item) => (
            <motion.div key={item.title} variants={itemAnim}>
              <Link href={item.slug ? `/post/${item.slug}` : "#"} className={`group block ${variant === "carousel" ? "min-w-[280px] sm:min-w-[340px] snap-start" : ""} rounded-2xl border border-slate-100 bg-white p-5 sm:p-6 shadow-lg shadow-black/[0.02] transition-all active:scale-[0.98] hover:-translate-y-1 hover:border-brand/30 hover:shadow-xl dark:border-white/10 dark:bg-white/10`}>
                <div className="flex items-start justify-between gap-4">
                  <span className="rounded-full bg-brand/10 px-3 py-1 text-xs font-bold text-brand">{item.category}</span>
                  <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-slate-50 text-slate-400 transition group-hover:bg-brand/10 group-hover:text-brand dark:bg-white/5 dark:group-hover:bg-brand/20">
