...
export function LiquidEther({
  ...
  useEffect(() => {
    ...
    let activeColors = colors && colors.length > 0 ? colors : [color0, color1, color2].filter(Boolean);
    if (!activeColors.length) activeColors = ["#0E877D", "#3FA8A5", "#5EEAD4"];

    // Palette texture
    function makePaletteTexture(stops: string[]) {
      let arr = stops;
      if (!Array.isArray(stops) || stops.length === 0) {
        arr = ["#0E877D", "#3FA8A5", "#5EEAD4"];
    ...
    // Autonomous demo animation driver
    class AutoDriver {
      mouse: MouseClass;
    ...
    class Simulation {
      ...
      update() {
        ...
        this.pressure.update({ vel, pressure: press });
      }
    ...
    class WebGLManager {
      ...
      dispose() {
        ...
        if (Common.renderer) {
          const dom = Common.renderer.domElement;
    ...
    try {
      ...
      console.warn("LiquidEther initialization failed:", e);
    }
